...
- **Focused expertise**: Each subagent specializes in a specific domain or type of task
- **Separate context**: Subagents have their own context windows, preserving the main conversation's context
- **Custom tools**: Control which tools each subagent can access
- **Tailored behavior**: Custom system prompts guide subagent behavior for specific use cases
...
3. Follow the guided setup to define your subagent
...
| `model` | No | Model to use for this subagent. Can be a model alias (`sonnet`, `opus`, or `haiku`) or `'inherit'` to use the main conversation's model. If omitted, defaults to the configured subagent model |
